...
public class CartRepository : ICartRepository
{
    ...
    public async Task Save(Domain.Cart aggregate, CancellationToken cancellationToken = default)
    {
        var cartFromDb = await _dbContext
                         ...
                         throw new CartNotFoundException(aggregate.Id);

        _dbContext.Entry(cartFromDb).CurrentValues.SetValues(aggregate);

        foreach (var cartItem in cartFromDb.Items)
        {
            if (aggregate.Items.All(x => x.Id != cartItem.Id))
            {
                _dbContext.CartItems.Remove(cartItem);
            }
        }

        foreach (var cartItemNewState in aggregate.Items)
        {
            var cartItemFromDb = cartFromDb.Items.SingleOrDefault(x => x.Id == cartItemNewState.Id);

            if (cartItemFromDb != null)
            {
                _dbContext.Entry(cartItemFromDb).CurrentValues.SetValues(cartItemNewState);
            }
            else
            {
                _dbContext.CartItems.Add(cartItemNewState);
            }
        }
